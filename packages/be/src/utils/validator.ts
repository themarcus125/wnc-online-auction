import { getOtpLength } from '@/user/otp/otp.service';
...
export const numberBetWeenRange = (val: number, down?: number, up?: number) => {
  if (down && down > val) return false;
  if (up && up < val) return false;
  return true;
};
